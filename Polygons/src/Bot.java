...
public class Bot extends Player{
    ...
    void wander(){
        if(this.moving_timer <= 0){
            this.speed = this.speed.add(new Vec2d((Math.random()*10-5)/1000f, (Math.random()*10-5)/1000f));//???
            this.moving_timer = 3;
        }
    }
    void follow(Player player){
        if(moving_timer <= 0){
            this.speed = this.speed.add(new Vec2d((player.pos.getX()-this.pos.getX())/1000f, (player.pos.getY()-this.pos.getY())/1000f));
            this.moving_timer = 3;
        }
    }
