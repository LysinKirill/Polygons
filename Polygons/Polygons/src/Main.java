...
public class Main extends JComponent implements KeyListener, ActionListener, MouseMotionListener, MouseListener, ComponentListener {
    ...
    public static void main(String[] args){
        ...
        while(true){    //    game cycle
            ...
            System.out.println(player.speed.getX());
            System.out.println(player.speed.getY());
            System.out.println("\n");
...
//            }
            ...
            //bot AI
            for (int i = 0;i < bots.size();i++){
                bots.get(i).wander(player, food.get((int)(Math.random()*food.size())));
            }
            //bullets moving
    ...
    public void keyPressed(KeyEvent keyEvent) {
        if(keyEvent.getKeyCode() == KeyEvent.VK_W){
            player.speed.setY(player.speed.getY()+0.1);
        }
        if(keyEvent.getKeyCode() == KeyEvent.VK_S){
            player.speed.setY(player.speed.getY()-0.1);
        }
        if(keyEvent.getKeyCode() == KeyEvent.VK_A){
            player.speed.setX(player.speed.getX()-0.1);
        }
